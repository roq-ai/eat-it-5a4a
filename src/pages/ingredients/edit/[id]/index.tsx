...
import { getIngredientById, updateIngredientById } from 'apiSdk/ingredients';
...
function IngredientEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<IngredientInterface>(
  ...
  const handleSubmit = async (values: IngredientInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateIngredientById(id, values);
      mutate(updated);
      resetForm();
