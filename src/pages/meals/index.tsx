...
type ColumnType = ColumnDef<MealInterface, unknown>;

interface MealListPageProps {
...
export function MealListPage(props: MealListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getMeals({
        relations: ['restaurant', 'constraint.count', 'ingredient.count', 'order.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<MealInterface>>(
    () => `/meals?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('ingredient', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.ingredient,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/meals/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
