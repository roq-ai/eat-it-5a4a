...
import { getConstraints } from 'apiSdk/constraints';
...
function DashboardPage() {
  ...
  const constraintStat = useSWR<PaginatedInterface<ConstraintInterface>>(
    () => `/constraints`,
    () => getConstraints({ limit: 1, offset: 0 }),
  );

  const mealStat = useSWR<PaginatedInterface<MealInterface>>(
    () => `/meals`,
    () => getMeals({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Constraints',
      result: constraintStat?.data?.totalCount,
      isLoading: constraintStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Meals',
      result: mealStat?.data?.totalCount,
      isLoading: mealStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
